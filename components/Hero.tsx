...
const trustItems = [
  ...
  { icon: MapPin, label: '47 Katy Businesses Ranked' },
...
export default function Hero({ className = '' }: { className?: string }) {
  return (
    <section
      className={`relative min-h-[90vh] flex items-center bg-primary bg-hero-texture ${className}`}
    ...
    >
      <div
        ...
        style={{
          backgroundImage:
            'repeating-linear-gradient(135deg, #fff 0px, #fff 1px, transparent 1px, transparent 50%)',
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24 pt-32">
        <div className="max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-white/10 text-slate-300 text-sm font-medium px-3 py-1.5 rounded-full mb-6 border border-white/20">
          ...
          <p className="text-lg sm:text-xl text-slate-300 leading-relaxed mb-8 max-w-2xl">
            ...
            competitors who&apos;ve been online longer. No agency fluff — just a specialist who
            knows this market.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-12">
            <Link
              href="/contact"
              className="inline-flex items-center justify-center px-6 py-3.5 bg-accent text-white font-semibold text-base rounded-md hover:bg-blue-700 transition-colors shadow-lg"
            >
              Get Your Free Local SEO Audit
            </Link>
            <Link
              href="/local-seo-services-katy"
              className="inline-flex items-center justify-center px-6 py-3.5 bg-white/10 text-white font-semibold text-base rounded-md hover:bg-white/20 transition-colors border border-white/30"
            >
              See How It Works
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {trustItems.map((item) => (
              ...
              >
                <item.icon size={16} className="text-trust flex-shrink-0" />
